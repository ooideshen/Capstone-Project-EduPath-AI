...
interface EditUniversityModalProps {
  open: boolean;
  handleClose: () => void;
  uniData: University | null; 
  onSuccess: () => void;
}

export default function EditUniversityModal({ open, handleClose, uniData, onSuccess }: EditUniversityModalProps) {
    ...
    const handleSubmit = async () => {
        if (!uniData?.id) {
            toast.error("Invalid university data");
            return;
        }
        ...
        try {
            ...
            const response = await fetch(`${API_URL}/api/university/update/${uniData.id}`, {
                method: 'PUT',
